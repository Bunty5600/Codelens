...
export default function Upload() {
 ...
 const handleAnalyze = async () => {
  if (!canAnalyze) {
    setAlert({
      type: 'warning',
      message: 'Please upload a file or paste code first.'
    })
    return
  }
  ...
  try {
    ...
    if (file) {
      ...
      const response = await fetch(
        'http://localhost:9000/analyze/upload',
        {
          method: 'POST',
          body: formData,
        }
      ...
      const response = await fetch(
        'http://localhost:9000/analyze/code',
        {
          method: 'POST',
          headers: {
            ...
            <div className="card p-6 space-y-6">
              <div>
                ...
                <p className="text-sm text-slate-500 dark:text-slate-400 mt-1">
                  Upload a .py / .js / .ts / .zip file or paste code directly.
                </p>
              </div>
